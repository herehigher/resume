...
function assertSemanticContract(contract, content, metadata, options) {
  ...
  if (contract.semantic === 'json-schema') {
    ...
    if (schema.$id !== `${DEPLOYMENT_ORIGIN}${contract.artifactPath}` || schema.title !== 'Resume Studio web v3 export') {
  ...
  if (contract.semantic === 'import-example') {
    ...
    if (example.version !== 3) failure(contract, metadata, 'version is invalid');
    return;
  }
