...
Open `http://localhost:8000/` in Chrome to start the Japanese editor directly or choose the Simplified Chinese or English introduction. You can also open the editor at `http://localhost:8000/editor/`.
...
1. Start the Japanese editor directly from the public homepage, or open the matching editor from the Simplified Chinese or English introduction. In the editor, use the language selector in the upper-right corner. You can also open `/editor/?lang=ja`, `/editor/?lang=zh-CN`, or `/editor/?lang=en` directly.
...
6. To delete saved data, switch to 日本語, select “保存した下書きを削除” at the bottom, and confirm the dialog. This removes the complete current v3 draft and on-screen input for all three languages.
...
- The current draft key is `resume-studio-web-v4`, using the same `version: 4` as imported and exported JSON. It stores the profile, photo, and all three language documents together. Only `resume-studio-web-v3` is read as a compatible prior key in read-only mode; the old draft and encryption key are removed only after the new-key save succeeds, while save failures, conflicts, and read-only fallback retain them. Out-of-range keys are never read, changed, or deleted.
...
See [AGENTS.md](AGENTS.md) for repository rules, the [development guide](docs/development-guide.md) for setup and checks, [Contributing](CONTRIBUTING.md) for PRs, and the [release playbook](docs/release-playbook.md) for publication.
