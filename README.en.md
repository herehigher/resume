...
- The current draft key is `resume-studio-web-v3`, using the same `version: 3` as imported and exported JSON. It stores the profile, photo, and all three language documents together. Only `resume-studio-web-v2` is read as a compatible prior key; out-of-range keys are never read, changed, or deleted, and the old draft and encryption key are removed after the new-key save succeeds.
