...
公开 Demo 正在准备中（Issue #9）。在公开地址完成验证前，请使用下面的本地启动方式。
...
- 应用会读取同一 origin 的静态 asset。用户主动点击 preview 中的 profile link 时会访问目标网站。除此之外，应用不会自动把简历输入提交给 backend 或外部服务。
...
请勿在正式 runtime 中加入外部 API、CDN、外部 font 或 analytics。详情参见 [Contributing](CONTRIBUTING.md) 和 [release acceptance checklist](docs/acceptance-checklist.md)。
