...
6. 保存データを削除するときは日本語画面の最下部で「保存した下書きを削除」を選び、確認 dialog で消去します。この操作は現在の v3 形式に保存された三言語すべての下書きと画面入力を削除します。
...
- 現在の草稿 key は `resume-studio-web-v3` で、JSON import/export と同じ `version: 3` を使います。profile、写真、三言語の文書をまとめて保存し、`resume-studio-web-v2` だけを互換 key として読取ります。対応範囲外の旧 key は読込・変更・削除しません。新 key への保存成功後に旧草稿と鍵を自動削除します。
...
- アプリ内削除は現在の v3 state だけを消します。無関係な storage 項目、対応範囲外の旧草稿、download 済み JSON/PDF、browser の download 履歴は削除しません。
