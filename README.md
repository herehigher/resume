...
6. 保存データを削除するときは日本語画面の最下部で「保存した下書きを削除」を選び、確認 dialog で消去します。この操作は現在の v4 形式に保存された三言語すべての下書きと画面入力を削除します。
...
- 現在の草稿 key は `resume-studio-web-v4` で、JSON import/export と同じ `version: 4` を使います。profile、写真、三言語の文書をまとめて保存し、`resume-studio-web-v3` だけを互換 key として read-only で読取ります。新 key への保存成功後にだけ旧草稿と鍵を自動削除し、保存失敗・競合・read-only fallback 時は旧草稿を保持します。対応範囲外の旧 key は読込・変更・削除しません。
