...
- 保存形式は `resume-studio-web-v4`。端末内草稿と JSON import/export は同じ単一の `version` を使う。`resume-studio-web-v3` だけを明示した互換 key として read-only で読み、v4 への保存成功後にだけ旧 raw と鍵を削除する。保存失敗、競合、Web Locks の read-only fallback 時は v3 草稿を保持する。対応範囲外や未設定の key は列挙・読込・変更・削除しない。暗号化草稿の保護を維持し、現在の草稿を意図的に削除する場合は利用者に確認する。
