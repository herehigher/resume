...
async function writeAssetFixture(root, {
  ...
  for (const [locale, expected] of Object.entries(outputs)) {
    ...
    for (const [kind, relativePath] of [['screenshot', expected.screenshotPath], ['pdf', expected.pdfPath]]) {
      const contents = Buffer.from(`${locale}-${kind}-${contentVariant}-fixture`);
...
test('committed LFS assets may come from another run when version, site, and semantic contracts match', async (t) => {
  ...
  await writeAssetFixture(committedRoot, { commit: 'b'.repeat(40), contentVariant: 'committed', lfsPointers: true });
...
test('release asset comparison rejects stale versions, site bytes, and LFS objects', async (t) => {
  ...
  await assert.rejects(compare(), /site bytes differ/);
  ...
  await assert.rejects(compare(), /committed ja screenshot bytes do not match its manifest/);

  await writeAssetFixture(committedRoot, { commit: 'b'.repeat(40), lfsPointers: true });
  await assert.rejects(
    compareReleaseAssets({ committedRoot, generatedRoot, sourceSha: 'd'.repeat(40) }),
    /generated source commit.*does not match Quality source/
  );
});
...
test('release asset comparison reports semantic drift separately from byte variation', async (t) => {
  ...
  await writeAssetFixture(committedRoot, {
    browserVersion: '122.0.0.0', commit: 'b'.repeat(40), lfsPointers: true
  });
