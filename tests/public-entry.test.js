...
test('default Japanese entry opens the editor directly and the legacy Japanese URL consolidates to it', () => {
  ...
  assert.match(root, /href="\.\/schema\/resume-studio-web-v4\.schema\.json"/);
...
test('published JSON Schema accepts exports and rejects primary invalid values', () => {
  const schema = JSON.parse(source('site/schema/resume-studio-web-v4.schema.json'));
  const example = JSON.parse(source('site/schema/resume-studio-web-v4.example.json'));
