...
test('development docs cover localized public entry and machine-readable contracts', () => {
  ...
  assert.equal(hasAcceptanceItem('Root', '/zh-cn/', '/en/', 'editor CTA', 'JSON Schema link'), true);
  assert.equal(hasAcceptanceItem('Root', 'JavaScript', 'H1', '?lang=ja'), true);
  assert.equal(hasAcceptanceItem('/ja/', 'canonical', 'root'), true);
...
test('changelog freezes dated releases without losing release notes', () => {
  ...
  const priorReleaseIndex = changelog.indexOf(priorRelease);
  ...
  assert.ok(priorReleaseIndex > latestReleaseIndex, 'Releases must remain in descending order');
  ...
  const latestReleaseNotes = changelog.slice(latestReleaseIndex + latestRelease.length, priorReleaseIndex);
  assert.match(latestReleaseNotes, /masked repository secret.*tagged module API/is);

  const priorReleaseNotes = changelog.slice(priorReleaseIndex + priorRelease.length, previousReleaseIndex);
  for (const fact of [
    '壊れた暗号化下書き',
    'X 連絡先',
    'マーモット',
  ...
  ]) assert.match(priorReleaseNotes, new RegExp(literalPattern(fact)));
...
test('release playbook is canonical and covers publication, evidence, and recovery decisions', () => {
  ...
  for (const contract of [
    ...
    /Git archive.*temporary bundle.*PNG \/ PDF content check/s,
    /--bundle.*--owner-approval/s,
    /backup.*全対象の復元.*backup を保持/s,
