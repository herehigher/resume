...
export function renderJapaneseCareer(state) {
  ...
  const careers = state.documents.ja.careers
    .map((career) => ({
      ...career,
      detailSections: Array.isArray(career.detailSections) ? career.detailSections : []
    }))
    ...
    .map((career) => {
      ...
      const detailSections = career.detailSections
        ...
        .join('');
      return `
    <section class="career-company" data-record-id="${escapeHTML(career.id)}" data-layout-mode="${layoutMode}">
