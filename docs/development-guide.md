...
CI の確認用出力は一時 directory から Actions artifact に保存し、通常の開発 PR では source / 展示物へ promote しません。Version を変えずに展示 asset だけを変更した PR は拒否します。安定版の公開 PR だけは、候補 branch の clean checkout で `npm run promote:pr-doc-assets -- --pr PR_NUMBER`（または `--quality-run-id QUALITY_RUN_ID`）を実行し、その PR の current merge commit と Quality artifact を一意に照合して検証済み asset を取り込みます。入口は temporary merge worktree で既存の `promote:doc-assets` を使い、候補 branch には screenshot 3件、PDF 3件、manifest の7 fileだけを反映します。候補の `site/`・package・generator input は commit 済みでなければならず、PR check は commit 済みの LFS object が promotion 元 artifact の exact bytes であることを確認します。Merge 後の公開準備は最終 main Quality に対し、各 file と manifest digest、候補 version、site・generator contract、PDF 全文・page、screenshot visual を再検査します。別 browser run の raw bytes 完全一致は要求せず、自動 commit も行いません。
